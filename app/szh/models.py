...
def get_obj_by_lid(lid):
    return db.session.query(ChatList).filter(ChatList.lid==lid).first()[0]

def updata_chatlist(lid,content,add_time):
    chat_dict = {"lid":lid,"content":content,"update_time":add_time}
